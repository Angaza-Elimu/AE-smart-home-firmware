...
Use the above format of Hybrid Approach - Arduino Library + Selective .a Precompiled Binaries to update this
arduino firmware source file into a library:
  - Secure sensitive information (server_url, root_cacert) into a separate .cpp (e.g., certificates.cpp) and declare them as
    extern constants in a header file (e.g., certificates.h).
  - Include comments in the library files where necessary:
  - The library folder to have the following structure
    MQ2SmokeMonitor/
    ├── examples/
    │   └── MQ2SmokeMonitorDemo/
    │       └── MQ2SmokeMonitorDemo.ino
    ├── src/
    │   ├── MQ2SmokeMonitor.cpp
    │   ├── MQ2SmokeMonitor.h
    │   ├── certificates.h          <-- Only declares extern const char*
    │   └── secrets.h               <-- Contains API_KEY, SSID, PASSWORD
    ├── lib/
    │   └── libcertificates.a          <-- Compiled binary with root_cacert and server_url
    └── library.properties

  - Print out each file library independently
